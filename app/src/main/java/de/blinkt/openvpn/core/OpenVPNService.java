...
import za.ac.uct.cs.powerqope.util.TotalTraffic;
...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener {
    public static final String NOTIFICATION_CHANNEL_ID = "remoteVpnService";
    ...
    private static final int OPENVPN_STATUS = 3347;
    ...
    private void showNotification(final String msg, String tickerText, boolean lowpriority, long when, ConnectionStatus status) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            nbuilder = new Notification.Builder(this, NOTIFICATION_CHANNEL_ID);
        } else{
            nbuilder = new Notification.Builder(this);
        }

        if (mProfile != null)
            nbuilder.setContentTitle("Remote VPN");
        else
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
            startCustomForeground(notification);
        else
    ...
    private void startCustomForeground(Notification notification) {
        String channelName = "Remote VPN Service";
