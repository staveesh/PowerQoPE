...
public class DNSResponsePatcher {
	...
	private static final String TAG = "DNSResponsePatcher";
	...
	public static void init(Set filter) {
		FILTER = filter;
		try {
	...
	protected static void trafficLog(String client, short clss, short type, String host, String answer, int length) {
		if (answer != null)
			Log.i(TAG, client + ", "+ clss + ", A-" + type + ", " + host + ", " + answer + ", /Length:" + length);
		else
			Log.i(TAG, client + ", "+ clss + ", Q-" + type + ", " + host + ", " + "<empty>");

	}

	protected static void logNstats(boolean result, String host) {
		if (result == true)
			Log.i(TAG, "FILTERED:" + host);
		else
			Log.i(TAG, "ALLOWED:" + host);
